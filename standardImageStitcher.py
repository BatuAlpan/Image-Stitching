...
class standardImageStitcher:
    ...
    def stitchImages(self, images, ransacTol, keypointThr, distanceThr, showSteps=False):
        ...
        for i in range(len(images) - 1):
            ...
            imMatches = cv.drawMatches(image1, keypoints1, image2, keypoints2, matches[1:20], None,
            ...
            if flag == 0:
                for i in range(len(matches)):
                    if matches[i].distance > distanceThr:  # Points above a certain threshold are ignored to make the
                        # code run faster.
                        continue
                ...
                print("Images cannot be stitched. Current number of valid keypoints are " + str(len(impKey)) +
                      " and its less than the threshold of " + str(keypointThr) + ".")
                return None
            ...
            output = cv.warpPerspective(image2, homograhyMat, (max(max_x, im1_x), max(max_y, im1_y)), dst=output,
                                        ...
                                        borderValue=cv.BORDER_CONSTANT)  # Image 2 is warped via the homography matrix.
